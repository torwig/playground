...
func main() {
	...
	go consumeMessagesWithTopic(ctx, &wg, conn, exchange, topic)
...
func consumeMessagesWithTopic(ctx context.Context, wg *sync.WaitGroup, conn *amqp.Connection, exchange string, topic string) {
	defer wg.Done()
	...
	for {
		...
		case msg, ok := <-msgCh:
			...
			log.Printf("consumer with topic '%s' received: %s", topic, string(msg.Body))
		}
	}
}
